...
class WPOnlineBackup_Backup_Transmission
{
	...
	/*public*/ function WPOnlineBackup_Backup_Transmission( & $WPOnlineBackup, $db_force_master = '' )
	{
		global $wpdb;
	...
	/*private*/ function Return_Error( $file, $line, $xml, $parser_ret = false )
	{
		return __( 'An online request failed: The server response was malformed. Please try again later.' , 'wponlinebackup' ) . PHP_EOL .
			__( 'If the issue persists, contact support and provide the following diagnostic information:' , 'wponlinebackup' ) . PHP_EOL . PHP_EOL .
	...
	/*public*/ function Validate_Account()
	{
		...
		if ( ( $ret = $this->Get( 'Status', $q, $xml, TRANSMISSION_NOTINBACKUP | TRANSMISSION_RETURNERRORS ) ) !== true ) {
			...
			// This will capture maintenance messages too
			if ( !is_numeric( $ret ) )
				return $ret;

			if ( $xml[0] == API_ERROR_WPONLINEBACKUP_KEY_MISMATCH )
				return 0;

			if ( $xml[0] == API_ERROR_LOGIN_FAILURE )
				return sprintf( __( 'Failed to login to the online vault: %s' , 'wponlinebackup' ), $xml[1] );
		...
		if ( !isset( $xml->data->Status )
			...
			return $this->Return_Error( __FILE__, __LINE__, $xml );
	...
	/*private*/ function Synchronise()
	{
		...
		if ( $this->job['progress'] == 0 ) {
			...
			// Force update so the message is updated
			...
			$this->stream->Impose_DataSize_Limit( $max, sprintf( __( 'The total backup size exceeds your maximum online quota of %s, and therefore cannot be transmitted to the online vault.', 'wponlinebackup' ), WPOnlineBackup_Formatting::Fix_B( $max, true ) ), __( 'The backup file is too large to send to the online vault. (%s).', 'wponlinebackup' ) );
			...
			// Force update so the message is updated and we don't duplicate event logs
			$this->bootstrap->Tick( false, true );
		...
		while ( $this->job['progress'] < 100 ) {
			...
			if ( $xml->data->SynchroniseGenerations[0]->_attr->Final == 1 ) {
				...
				// Force update to ensure we don't duplicate log entries
				$this->bootstrap->Tick( false, true );
	...
	/*private*/ function Transmit()
	{
		...
		if ( $this->job['progress'] == 0 ) {
			...
			$this->bootstrap->Log_Event(
				WPONLINEBACKUP_EVENT_INFORMATION,
				sprintf( __( 'The backup is %s in size. Starting transmission to the online vault.', 'wponlinebackup' ), WPOnlineBackup_Formatting::Fix_B( $this->job['total'], true ) )
			);
		...
		while ( $this->job['progress'] < 99 ) {
			...
			if ( $xml->data->PullStatus[0]->_attr->Complete == '1' ) {
				...
				if ( isset( $xml->data->PullStatus[0]->_text )
					|| !isset( $xml->data->PullStatus[0]->_attr->BusySignal ) )
					return $this->bootstrap->MALError( __FILE__, __LINE__, $xml );
				...
				if ( $this->job['done_retention'] )
					...
					$this->bootstrap->Log_Event(
						WPONLINEBACKUP_EVENT_INFORMATION,
						__( 'Retention is not required, you are still within your maximum quota.' , 'wponlinebackup' )
					);
	...
	/*private*/ function Request( $method, $api, $data, & $xml, $body, $flags )
	{
		...
		$data['blogurl']	= !( $flags & TRANSMISSION_NOTINBACKUP ) ? $this->progress['cache']['blogurl'] : $this->WPOnlineBackup->Get_Blog_URL();
		...
		if ( ( $ret = $xml->fetch( $response['body'] ) ) !== true ) {
			...
			return $this->Return_Error( __FILE__, __LINE__, $xml, $ret );
		...
		if ( !isset( $xml->data->Error )
			...
			if ( isset( $xml->data->Maintenance[0] )
				&& isset( $xml->data->Maintenance[0]->_attr->Type )
				&& isset( $xml->data->Maintenance[0]->Message[0]->_text ) ) {

				// Flag up maintenance in the bootstrap
				$type = intval( $xml->data->Maintenance[0]->_attr->Type );
				if ( !( $flags & TRANSMISSION_RETURNERRORS ) )
					$this->bootstrap->maintenance = $type;

				// Grab the correct message string
				switch ( $type ) {
					//case WPONLINEBACKUP_MAINTENANCE_SCHEDULED:
					default:
						$message = __( 'The online backup vault is currently down for scheduled maintenance. We apologise for any inconvenience. %s', 'wponlinebackup' );
						break;
					case WPONLINEBACKUP_MAINTENANCE_EMERGENCY:
						$message = __( 'The online backup vault is currently down for emergency maintenance. We apologise for any inconvenience. %s', 'wponlinebackup' );
						break;
				}

				// Return the maintenance message
				return sprintf( $message, $xml->data->Maintenance[0]->Message[0]->_text );
