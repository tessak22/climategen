...
HEADER;
	...
	/*public*/ function Load( $state, $rotation = false )
	{
		...
		$this->rc_header = $state['rc_header'];
		$this->has_header = $state['has_header'];

		$this->rotate_id = $state['rotate_id'];
		if ( $rotation === false )
			$rotate = false;
		else
			$rotate = $rotation != $this->rotate_id ? true : false;
		...
		if ( $this->status == 0 && $rotate ) {
			...
			// All rotations have rejection header
		...
		if ( $this->status == 0 ) {
			...
			// Reopen the file
			if ( ( $this->file = @fopen( $this->prefix . '/' . $state['name'] . '.php', $rotate ? 'w+b' : 'r+b' ) ) === false )
				return OBFW_Exception();

			// Attempt to truncate the file to the correct size
			@ftruncate( $this->file, ( $this->has_header ? strlen( $this->header ) : 0 ) + $this->size );
			...
			if ( $rotate || $this->size == 0 ) {
				...
				// Write the Rejection header if we want one
				if ( $this->has_header && true !== ( $ret = $this->Write_Rejection_Header( $this->file ) ) ) {
					@fclose( $this->file );
					return $ret;
				}
				...
				if ( @fseek( $this->file, ( $this->has_header ? strlen( $this->header ) : 0 ) + $this->size, SEEK_SET ) != 0 ) {
					@fclose( $this->file );
					return OBFW_Exception();
				}
			...
			if ( ( $this->rc_file = @fopen( $this->prefix . '/' . $state['name'] . '.rc.php', 'r+b' ) ) === false )
				return OBFW_Exception();

			// Attempt to truncate the file to the correct size
			@ftruncate( $this->rc_file, ( $this->rc_header ? strlen( $this->header ) : 0 ) + $this->rc_offset );
			...
			if ( $this->rc_offset == 0 ) {
				...
				// Write the Rejection header if we want one in the rc file
				...
				// Seek to where we left off
				if ( @fseek( $this->rc_file, ( $this->rc_header ? strlen( $this->header ) : 0 ) + $this->rc_offset, SEEK_SET ) != 0 ) {
					@fclose( $this->rc_file );
					return OBFW_Exception();
				}
	...
	/*public*/ function Open( $name, $rjct_header = true )
	{
		// ASSERTION - Status is null and we are initialised
		$this->status = 0;

		// Set the rejection header
		$this->Get_Header();

		// Modify the name to include a random seed so it is hidden from the world
		$this->name = $name = $name . '.' . $this->WPOnlineBackup->Random_Seed( $name, 'disk' );

		if ( $this->rotate_id )
			$name .= '.' . $this->rotate_id;

		// Create the file with PHP extension so you cannot download it
		if ( ( $this->file = @fopen( $this->prefix . '/' . $name . '.php', 'w+b' ) ) === false )
			return OBFW_Exception();

		$this->rc_header = $rjct_header;
		$this->has_header = $rjct_header;

		// Write the Rejection header if we want one on the initial file
		if ( $this->has_header && true !== ( $ret = $this->Write_Rejection_Header( $this->file ) ) ) {
			@fclose( $this->file );
			return $ret;
		}
	...
	/*private*/ function Partial_Write( $method, $written, $length, $type = '' )
	{
		$e = OBFW_Exception();
		$name = $this->name;
		switch ( $type ) {
		...
		return 'Attempt to ' . $method . ' to file ' . $this->prefix . '/' . $name . '.php only partially succeeded. Only ' . $written . ' of ' . $length . ' bytes were written. (' . $this->size . ' bytes already written.) ' . $e;
	}

	/*private*/ function _Try_Write( $file, $data, $length, $partial, $partial_type = '' )
	{
		...
		// Interestingly we ARE seeing instances of 0 getting returned
		// One cause we have confirmed is the file reached 2GB and PHP is 32-bit with the bodged large file support disabled
		// However, we still see it even with arbitrary bytes getting written - so that is why we retry with a pause

		// Another interesting case we are seeing is sometimes the file can reach a certain size and fwrite just crashes the script without reason
		// This has a completely unknown cause
		$todo_length = $length;
		...
		while ( true ) {
			...
			if ( $written == 0 ) {
				...
				if ( --$tries <= 0 )
					return $this->Partial_Write( $partial, $length - $todo_length, $length, $partial_type );
	...
	/*public*/ function Write( $data, $length = null )
	{
		...
		if ( is_null( $length ) )
			...
			return 'Attempt to write file ' . $this->prefix . '/' . $name . '.php failed due to insufficient data. Only ' . $rlen . ' of ' . $length . ' bytes were available.';

		// Nothing to write?
		if ( $length == 0 )
			return true;

		if ( true !== ( $ret = $this->_Try_Write( $this->file, $data, $length, 'write' ) ) )
			return $ret;
	...
	/*public*/ function Rewrite( $offset, $data, $length = null )
	{
		...
		if ( $offset >= $this->offset ) {
			...
			if ( @fseek( $this->file, ( $this->has_header ? strlen( $this->header ) : 0 ) + ( $offset - $this->offset ), SEEK_SET ) != 0 ) return OBFW_Exception();

			if ( true !== ( $ret = $this->_Try_Write( $this->file, $data, $length, 'rewrite' ) ) )
				return $ret;

			if ( @fseek( $this->file, ( $this->has_header ? strlen( $this->header ) : 0 ) + $this->size, SEEK_SET ) != 0 ) return OBFW_Exception();
			...
			// Write the patch
			if ( true !== ( $ret = $this->_Try_Write( $this->p_file, $data, $this_length, 'rewrite', 'patch' ) ) )
				return $ret;
	...
	/*public*/ function Flush( & $buffer, $length )
	{
		...
		foreach ( $this->rotation as $id => $rotate ) {
			...
			if ( $rotate['offset'] + $rotate['size'] > $this->d_offset + $flushed ) {
				...
				if ( ( $to_flush = min( $length - $flushed, $available ) ) != 0 ) {
					...
					if ( @fseek( $this->r_files[$id], ( $rotate['has_header'] ? strlen( $this->header ) : 0 ) + $this->d_offset + $flushed - $rotate['offset'], SEEK_SET ) != 0 ) return OBFW_Exception();
		...
		if ( $this->offset + $this->size > $this->d_offset + $flushed ) {
			...
			if ( ( $to_flush = min( $length - $flushed, $available ) ) != 0 ) {
				...
				// Get the data
				if ( @fseek( $this->file, ( $this->has_header ? strlen( $this->header ) : 0 ) + $this->d_offset + $flushed - $this->offset, SEEK_SET ) != 0 ) return OBFW_Exception();
	...
	/*public*/ function Do_Reconstruct()
	{
		if ( $this->status == 3 ) {
			...
			return array(
				...
				'offset'	=> $this->rc_header ? strlen( $this->header ) : 0,
		...
		if ( true !== ( $ret = $this->_Try_Write( $this->rc_file, $data, $length, 'write', 'rc' ) ) ) {
			@fclose( $this->rc_file );
			return $ret;
		}
	...
	/*private*/ function Write_Rejection_Header( $file, $type = '' )
	{
		...
		if ( true !== ( $ret = $this->_Try_Write( $file, $data, $length, 'write header', $type ) ) )
			return $ret;
	...
	/*private*/ function CleanUp_Rotated()
	{
		...
		foreach ( $this->rotation as $id => $rotate ) {
			...
			if ( $rotate['size'] == 0 ) {
				...
				// Attempt to truncate the file to the correct size
				@ftruncate( $file, ( $rotate['has_header'] ? strlen( $this->header ) : 0 ) + $rotate['size'] );
