...
class WPOnlineBackup_Stream_Delta
{
	...
	/*public*/ function Open( $config, $title, $description )
	{
		...
		// Default to 2GB limits - fancy math to avoid float conversions
		$this->datasize_limit = array(
			( 1024*1024*1024 - 2 )*2 + 3,
			__( 'The backup is larger than the allowed size of %s (%s). Try reducing the size of your backup by adding some exclusions.', 'wponlinebackup' ),
			__( 'The backup is too big to send to the online backup vault (%s).', 'wponlinebackup' )
		);

		$this->filesize_limit = array(
			( 1024*1024*1024 - 2 )*2 + 3,
			__( 'The file is larger than the maximum allowed size of %s (%s)', 'wponlinebackup' )
		);
	...
	/*public*/ function CleanUp( $wipe = true )
	{
		if ( $this->status == 0 || $this->status == 1 ) {
			...
			$this->writer->CleanUp( $wipe );
		}

		$this->data_disk->CleanUp( $wipe );
		$this->indx_disk->CleanUp( $wipe );
	}
	...
	/*public*/ function Impose_DataSize_Limit( $size, $message = false, $ret = false )
	{
		$this->datasize_limit[0] = $size;

		if ( $message !== false ) {
			$this->datasize_limit[1] = $message;
			$this->datasize_limit[2] = $ret;
		}
	}

	/*public*/ function Impose_FileSize_Limit( $size, $message = false )
	{
		$this->filesize_limit[0] = $size;

		if ( $message !== false )
			$this->filesize_limit[1] = $message;
	}
	...
	/*public*/ function Add_File_From_Path( $bin, $file, $path, & $size, $status )
	{
		...
		if ( $size > $this->filesize_limit[0] ) {
		...
		} else if ( 4 + 44 + strlen( $file ) > $this->datasize_limit[0] - $this->indx_disk->Pos() ) {
			...
			$size = array(
				sprintf( $this->datasize_limit[1], WPOnlineBackup_Formatting::Fix_B( $this->datasize_limit[0] ), WPOnlineBackup_Formatting::Fix_B( $total_size ) ),
				sprintf( $this->datasize_limit[2], WPOnlineBackup_Formatting::Fix_B( $total_size ) ),
			);
			return false;
	...
	/*private*/ function _Add_File_From_String( $bin, $file, & $size, $data, $status )
	{
		...
		// Check size constraints
		if ( $result['zlen'] > $this->filesize_limit[0] ) {

			// Compressed size is bigger than what we have set as our maximum size!
			$size = sprintf( $this->filesize_limit[1], WPOnlineBackup_Formatting::Fix_B( $this->filesize_limit[0] ), WPOnlineBackup_Formatting::Fix_B( $result['zlen'] ) );
			return true;

		} else if ( $result['zlen'] > $this->datasize_limit[0] - $pos ) {

			// The compressed size sends the data disk over the data limit
			$total_size = $this->data_disk->Pos() + $result['zlen'];
	...
	/*public*/ function Write_Stream( $data, $len = false )
	{
		...
		if ( !$this->rolling_writing ) {
			...
			if ( $ret === false ) {
				...
				if ( true !== ( $ret = $this->writer->Open( $this->data_disk, $this->config['encryption'], $this->config['encryption_key'] ) ) )
					return $ret;
	...
	/*public*/ function Add_Deletion_Entry( $bin, $item_id, $backup_time, $deletion_time )
	{
